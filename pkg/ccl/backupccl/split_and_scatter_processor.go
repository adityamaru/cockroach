...
type splitAndScatterer interface {
	// splitAndScatterSpan issues a split request at a given splitKey if not nil
	// and then scatters the range around the cluster. It returns the node ID of
	// the leaseholder of the span after the scatter if known, 0 otherwise.
	splitAndScatter(ctx context.Context, codec keys.SQLCodec, db *kv.DB, kr *storageccl.KeyRewriter, splitKey, scatterKey roachpb.Key, randomizeLeases bool) (roachpb.NodeID, error)
}
...
func (n noopSplitAndScatterer) splitAndScatter(
...
// splitAndScatter implements the splitAndScatterer interface.
func (s dbSplitAndScatterer) splitAndScatter(
	ctx context.Context,
	codec keys.SQLCodec,
	db *kv.DB,
	...
	newScatterKey, err := rewriteBackupSpanKey(codec, kr, scatterKey)
	if err != nil {
		return 0, err
	}

	if len(splitKey) > 0 {
		newSplitKey, err := rewriteBackupSpanKey(codec, kr, splitKey)
		if err != nil {
			return 0, err
		}
		log.VEventf(ctx, 1, "presplitting new key %+v", newSplitKey)
		if err := db.AdminSplit(ctx, newSplitKey, expirationTime); err != nil {
			return 0, errors.Wrapf(err, "splitting key %s", newSplitKey)
		}
	}
